    ...
    private static void loadData() {
        try (BufferedReader reader = new BufferedReader(new FileReader("data.txt"))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (data.length == 3) { // Data akun (Admin atau Customer)
                    String id = data[0];
                    String username = data[1];
                    String password = data[2];
    
                    if (username.startsWith("admin")) { // Jika akun adalah Admin
                        currentAdmin = new Admin(id, username, password);
                        accounts.add(currentAdmin);
                    } else { // Jika akun adalah Customer
                        accounts.add(new Customer(id, username, password));
                    }
                } else if (data.length == 4) { // Data produk
                    if (currentAdmin != null) { // Pastikan admin aktif
                        ...
                        currentAdmin.addProduct(product); // Tambahkan produk ke admin aktif
                    }
                } else if (line.startsWith("#")) {
                    // Abaikan komentar (# Produk kosong untuk Admin ini)
                    continue;
                }
            }
